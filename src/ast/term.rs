...
pub enum CTerm {
    ...
    /// Note: the flag means whether the function has effects that need to be handled by some
    /// handlers. System effects like IO do not count because they appear to be a simple call. This
    /// value should be conservatively set to true if side effects are unknown. For example, the
    /// containing redex may not have enough arguments to determine the effects of this computation.
