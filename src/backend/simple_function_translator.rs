...
use crate::ast::term::{CTerm, CType, Effect, PType, SpecializedType, VTerm, VType};
use crate::backend::common::{
    BuiltinData, BuiltinFunction, FunctionFlavor, HasType, TypedReturnValue, TypedValue,
};
...
impl<'a, M: Module> SimpleFunctionTranslator<'a, M> {
    ...
    pub fn store_return_value_on_argument_stack(
        &mut self,
        return_value_or_param: TypedReturnValue,
        num_args: usize,
    ) -> Value {
        ...
        let return_address = self
            .function_builder
            .ins()
            .iadd_imm(self.base_address, return_address_offset);
        self.function_builder
            .ins()
            .store(MemFlags::new(), value, return_address, 0);
        return_address
    }
    ...
    pub fn new<F>(
        ...
        parameter_initializer: fn(
            &mut SimpleFunctionTranslator<M>,
            Block,
            usize,
            &VType,
        ) -> TypedReturnValue,
    ) -> SimpleFunctionTranslator<'a, M>
    where
        F: FnOnce(&mut FunctionBuilder, Block) -> Value,
    {
        compiler.ctx.clear();
        compiler.ctx.func.signature = sig;

        let mut function_builder =
            FunctionBuilder::new(&mut compiler.ctx.func, &mut compiler.builder_context);
        ...
        let tip_address_slot = function_builder.create_sized_stack_slot(StackSlotData::new(
        ...
        for (i, (v, v_type)) in function_definition.args.iter().enumerate() {
            translator.local_vars[*v] =
                parameter_initializer(&mut translator, entry_block, i, v_type);
        }
        translator
    }
    ...
    pub fn translate_c_term(&mut self, c_term: &CTerm, is_tail: bool) -> TypedReturnValue {
        match c_term {
            CTerm::Redex { .. } => {
                ...
                self.translate_redex(c_term, is_tail, Effect::Simple, |c_term, is_tail| unsafe {
            ...
            CTerm::Def { name, .. } => {
                ...
                if is_tail && !self.is_specialized {
                    ...
                    self.function_builder
                        .ins()
                        .return_call(func_ref, &[base_address]);
                    None
                } else {
                    let inst = self
                        .function_builder
                        .ins()
                        .call(func_ref, &[self.tip_address]);
            ...
            CTerm::MemGet { base, offset } => {
                ...
                let value = self
                    .function_builder
                    .ins()
                    .load(I64, MemFlags::new(), load_address, 0);
                Some((value, Uniform))
            }
            CTerm::MemSet {
                base,
                ...
                self.function_builder
                    .ins()
                    .store(MemFlags::new(), value_value, store_address, 0);
            ...
            CTerm::PrimitiveCall { name, args } => {
                let args = args
                    .iter()
                    .map(|arg| self.translate_v_term(arg))
                    .collect::<Vec<_>>();
                let primitive_function = *PRIMITIVE_FUNCTIONS.get(name).unwrap();
                let arg_values: Vec<Value> = primitive_function
                    .arg_types
                    .iter()
                    .zip(args)
                    .map(|(ty, arg)| self.adapt_type(arg, ty))
                    .collect();
                let return_value =
    ...
    pub fn translate_redex<F>(
        &mut self,
        c_term: &CTerm,
        is_tail: bool,
        context_effect: Effect,
        mut translate_c_term: F,
    ) -> TypedReturnValue
    where
        F: FnMut(&CTerm, bool) -> TypedReturnValue,
    {
        let CTerm::Redex { box function, args } = c_term else {
            unreachable!()
        };
        if let CTerm::Def { name, effect } = function {
            ...
            if let CType::SpecializedF(return_type) = return_type
                && arg_types.len() == args.len()
                && (effect.intersect(context_effect) != Effect::Complex)
            {
                let tip_address = self.tip_address;
                let all_args = iter::once(tip_address)
                    .chain(args.iter().zip(arg_types).map(|(arg, v_type)| {
        ...
        F: FnMut(&CTerm, bool) -> TypedReturnValue,
    {
        ...
        // return value
        self.function_builder
            .append_block_param(joining_block, result_value_type);
        // tip address
    ...
    pub fn translate_handler(
        ...
        let transform_loader_cps_impl_func_ptr = self
            .function_builder
            .ins()
            .func_addr(I64, transform_loader_cps_impl_func_ref);
        let tip_address_ptr = self.store_tip_address_to_stack();
        let inst = self.call_builtin_func(
            BuiltinFunction::RegisterHandler,
            &[
                tip_address_ptr,
                next_continuation,
                parameter_value,
        ...
        let transform_loader_continuation =
            self.function_builder
                .ins()
        ...
        let offset_in_bytes = self
            .function_builder
            .ins()
            .isub(old_tip_address, self.tip_address);
    ...
    pub fn adjust_continuation_height(&mut self, continuation: Value, offset_in_bytes: Value) {
        let continuation_height =
            self.function_builder
                .ins()
                .load(I64, MemFlags::new(), continuation, 8);
        let continuation_height_bytes =
            self.function_builder.ins().ishl_imm(continuation_height, 3);
        let new_continuation_height_bytes = self
            .function_builder
            .ins()
            .iadd(continuation_height_bytes, offset_in_bytes);
        let new_continuation_height = self
            .function_builder
            .ins()
            .ushr_imm(new_continuation_height_bytes, 3);
        self.function_builder.ins().store(
            MemFlags::new(),
            new_continuation_height,
            continuation,
            8,
        );
    }
    ...
    pub(crate) fn invoke_thunk(
        ...
        let sig_ref = self
            .function_builder
            .import_signature(self.uniform_cps_func_signature.clone());
        if is_tail && !self.is_specialized {
            let base_address = self.copy_tail_call_args_and_get_new_base();
            self.function_builder.ins().return_call_indirect(
                sig_ref,
                func_pointer,
                &[base_address, next_continuation],
            );
            None
        } else {
            let inst = self.function_builder.ins().call_indirect(
                sig_ref,
                func_pointer,
                &[self.tip_address, next_continuation],
            );
            self.extract_return_value(inst)
        }
    }
    ...
    pub fn process_thunk(&mut self, thunk: &VTerm) -> Value {
        ...
        let tip_address_ptr = self.store_tip_address_to_stack();
        let inst =
            self.call_builtin_func(BuiltinFunction::ForceThunk, &[thunk_value, tip_address_ptr]);
    ...
    fn load_tip_address_from_stack(&mut self) {
        self.tip_address = self
            .function_builder
            .ins()
            .stack_load(I64, self.tip_address_slot, 0);
    }

    fn store_tip_address_to_stack(&mut self) -> Value {
        self.function_builder
            .ins()
            .stack_store(self.tip_address, self.tip_address_slot, 0);
        let tip_address_ptr = self
            .function_builder
            .ins()
            .stack_addr(I64, self.tip_address_slot, 0);
        tip_address_ptr
    }
        ...
        F: FnMut(&CTerm, bool) -> TypedReturnValue,
    {
        ...
        match typed_return_value {
            ...
            Some(..) => {
                ...
                self.function_builder
                    .ins()
                    .jump(joining_block, &[value, self.tip_address]);
            }
        }
    }
    ...
    pub fn extract_return_value(&mut self, inst: Inst) -> TypedReturnValue {
        ...
        let return_value =
            self.function_builder
                .ins()
                .load(I64, MemFlags::new(), return_address, 0);
    ...
    pub fn copy_tail_call_args_and_get_new_base(&mut self) -> Value {
        ...
        let new_base_value = self
            .function_builder
            .ins()
            .iadd_imm(self.tip_address, (self.num_args * 8) as i64);
        let num_bytes_to_copy = self
            .function_builder
            .ins()
            .isub(self.base_address, self.tip_address);
        self.function_builder.call_memmove(
        ...
        match v_term {
            ...
            VTerm::Str { value } => {
                ...
                let data_id = self.static_strings.entry(value.clone()).or_insert_with(|| {
                    ...
                    let data_id = self
                        .module
                        .declare_data(value, Linkage::Local, false, false)
                        .unwrap();
                    self.module
                ...
                let global_value = self
                    .module
                    .declare_data_in_func(*data_id, self.function_builder.func);
    ...
    pub fn get_local_function(&mut self, name: &str, flavor: FunctionFlavor) -> FuncRef {
        ...
        let func_id = self
            .local_functions
            .get(&desired_func_name)
            .unwrap_or_else(|| panic!("Cannot get function '{}'", &desired_func_name));
        self.module
            .declare_func_in_func(*func_id, self.function_builder.func)
    }
    ...
    fn create_struct(&mut self, values: Vec<Value>) -> Value {
        ...
        let runtime_alloc_call =
            self.call_builtin_func(BuiltinFunction::Alloc, &[struct_size_value]);
    ...
    pub(crate) fn call_builtin_func(
        &mut self,
    ...
    pub(crate) fn get_builtin_data(&mut self, builtin_data: BuiltinData) -> Value {
        let data_ref = self
            .module
            .declare_data_in_func(self.builtin_data[builtin_data], self.function_builder.func);
        ...
        self.function_builder
            .ins()
            .iadd_imm(result, builtin_data.offset())
    }
            ...
            Specialized(s) => match s {
                ...
                SpecializedType::Primitive(p) => match p {
                    PType::I64 | PType::F64 => {
                        ...
                        self.function_builder
                            .ins()
                            .store(MemFlags::new(), value, ptr, 0);
                    ...
                    PType::F32 => {
                        let casted =
                            self.function_builder
                                .ins()
                                .bitcast(I32, MemFlags::new(), value);
    ...
    pub(crate) fn convert_to_special(
        &mut self,
        value_and_type: TypedReturnValue,
            ...
            Uniform => match specialized_type {
                ...
                SpecializedType::Primitive(p) => match p {
                    PType::I64 | PType::F64 => {
                        ...
                        self.function_builder
                            .ins()
                            .load(p.get_type(), MemFlags::new(), ptr, 0)
                    }
                    PType::I32 => {
                    ...
                    PType::F32 => {
                        ...
                        self.function_builder
                            .ins()
                            .bitcast(F32, MemFlags::new(), truncated)
                    }
                },
            },
            Specialized(s) => {
                if s == specialized_type {
                    value
                } else {
    ...
    pub fn handle_operation_call(
        ...
        let captured_continuation_record_impl_ref = self.module.declare_func_in_func(
            self.builtin_functions[BuiltinFunction::CapturedContinuationRecordImpl],
            self.function_builder.func,
        );
        let captured_continuation_record_impl_ptr = self
            .function_builder
            .ins()
            .func_addr(I64, captured_continuation_record_impl_ref);
        let simple_handler_runner_impl_ref = self.module.declare_func_in_func(
            self.builtin_functions[BuiltinFunction::SimpleHandlerRunnerImpl],
            self.function_builder.func,
        );
        let simple_handler_runner_impl_ptr = self
            .function_builder
            .ins()
        ...
        let handler_impl = self
            .function_builder
            .ins()
            .load(I64, MemFlags::new(), result_ptr, 0);
        let handler_base_address =
            self.function_builder
                .ins()
                .load(I64, MemFlags::new(), result_ptr, 8);
        let next_continuation =
            self.function_builder
                .ins()
                .load(I64, MemFlags::new(), result_ptr, 16);
        ...
        if use_return_call {
            self.function_builder.ins().return_call_indirect(
                sig_ref,
                handler_impl,
                &[handler_base_address, next_continuation],
            )
        } else {
