...
pub trait Visitor {
    ...
    fn with_binding(&self, ctx: &Self::Context, name: &str) -> Self::Context;

    fn visit_v_term(&self, ctx: &Self::Context, v_term: &VTerm) -> Self::Result {
        match v_term {
    ...
    fn visit_int(&self, _ctx: &Self::Context, _value: &i32) -> Self::Result {
        self.combine(vec! {})
    }

    fn visit_str(&self, _ctx: &Self::Context, _value: &String) -> Self::Result {
        self.combine(vec! {})
    }

    fn visit_tuple(&self, ctx: &Self::Context, values: &Vec<VTerm>) -> Self::Result {
        self.combine(values.iter().map(|v| self.visit_v_term(ctx, v)).collect())
    }

    fn visit_c_term(&self, ctx: &Self::Context, c_term: &CTerm) -> Self::Result {
        match c_term {
    ...
    fn visit_lambda(&self, ctx: &Self::Context, arg_name: &str, body: &CTerm) -> Self::Result {
        self.visit_c_term(&self.with_binding(ctx, arg_name), body)
    }

    fn visit_app(&self, ctx: &Self::Context, function: &CTerm, arg: &VTerm) -> Self::Result {
        self.combine(vec![
    ...
    fn visit_return(&self, ctx: &Self::Context, value: &VTerm) -> Self::Result {
        self.visit_v_term(ctx, value)
    }

    fn visit_force(&self, ctx: &Self::Context, thunk: &VTerm) -> Self::Result {
        self.visit_v_term(ctx, thunk)
    }
    fn visit_let(&self, ctx: &Self::Context, t: &CTerm, bound_name: &str, body: &CTerm) -> Self::Result {
        self.combine(vec![
            self.visit_c_term(ctx, t),
    ...
    fn visit_case_int(&self, ctx: &Self::Context, t: &VTerm, branches: &HashMap<i32, CTerm>, default_branch: &Option<Box<CTerm>>) -> Self::Result {
    ...
    fn visit_case_tuple(&self, ctx: &Self::Context, t: &VTerm, bound_names: &Vec<String>, branch: &CTerm) -> Self::Result {
        let mut results = vec![self.visit_v_term(ctx, t)];
    ...
    fn visit_case_str(&self, ctx: &Self::Context, t: &VTerm, branches: &HashMap<String, CTerm>, default_branch: &Option<Box<CTerm>>) -> Self::Result {
        let mut results = vec![self.visit_v_term(ctx, t)];
        for (_, branch) in branches {
            results.push(self.visit_c_term(ctx, branch));
        }
