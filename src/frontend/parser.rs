use crate::ast::term::{CType, Effect, PType, SpecializedType, VType};
...
fn identifier_token(input: Span) -> IResult<Span, Token> {
    context(
        ...
        alt((
            ...
            delimited(char('`'), take_while1(|c| c != '`'), char('`')).map(|s: Span| {
                Token::Normal(
                    s.to_string(),
                    input.location_line() - 1,
                    input.naive_get_utf8_column() - 1,
...
fn int_token(input: Span) -> IResult<Span, Token> {
    context(
        "int_token",
        map(take_while1(|c: char| c.is_ascii_digit()), |s: Span| {
            Token::Int(
                s.to_string().parse::<i64>().unwrap(),
                input.location_line() - 1,
                s.naive_get_utf8_column() - 1,
            )
        }),
    )(input)
}
...
fn tokens(input: Span) -> IResult<Span, Vec<Token>> {
    preceded(
        ...
        separated_list0(
            space0,
            alt((identifier_token, int_token, str_token, indent_token)),
        ),
    )(input)
}
...
fn boolean<I, P1, P2, O1, O2, E>(
    false_parser: P1,
    true_parser: P2,
) -> impl FnMut(I) -> IResult<I, bool, E>
where
    I: Clone + InputLength,
    P1: Parser<I, O1, E>,
    P2: Parser<I, O2, E>,
...
fn map_token<F, R>(f: F) -> impl FnMut(Input) -> IResult<Input, R>
where
    F: Fn(&Token) -> Option<R>,
{
    move |input: Input| {
            ...
            match f(token) {
                Some(r) => Ok((
                    Input {
                        ...
                        current_indent: input.current_indent,
                    },
                    r,
                )),
...
fn satisfy<F>(f: F) -> impl FnMut(Input) -> IResult<Input, ()>
where
    F: Fn(&Token) -> bool,
{
    move |input: Input| {
        if input.tokens.is_empty() {
            ...
            match f(token) {
                ...
                false => Err(nom::Err::Error(nom::error::Error {
                    input,
...
/// identifier.
fn scoped<'a, F, R>(mut f: F) -> impl FnMut(Input<'a>) -> IResult<Input<'a>, R>
where
    F: Parser<Input<'a>, R, nom::error::Error<Input<'a>>>,
{
    move |input| {
        if input.tokens.is_empty() {
            Err(nom::Err::Error(nom::error::Error {
...
fn v_type_decl(input: Input) -> IResult<Input, VType> {
    map(opt(preceded(token(":"), v_type)), |v_type_option| {
        v_type_option.unwrap_or(VType::Uniform)
    })(input)
}
...
fn newline(input: Input) -> IResult<Input, ()> {
    ...
    let mut p =
        satisfy(|token| matches!(token,Token::Indent(_, column) if *column >= current_indent ));
    p(input)
}
    ...
    map_token(|token| match token {
        Token::Normal(name, _, _)
            if KEYWORDS.iter().all(|k| k != name)
                && PRECEDENCE
                    .iter()
                    .all(|(names, ..)| names.iter().all(|(n, _)| n != name)) =>
        {
            Some(name.clone())
        }
        _ => None,
...
fn effect(input: Input) -> IResult<Input, Effect> {
    map(opt(token("!").map(|_| Effect::Complex)), |effect_opt| {
        effect_opt.unwrap_or(Effect::Simple)
    })(input)
}
...
fn id_term(input: Input) -> IResult<Input, FTerm> {
    context(
        "id_term",
        map(pair(id, effect), |(name, effect)| FTerm::Identifier {
            name,
            effect,
        }),
    )(input)
...
fn force(input: Input) -> IResult<Input, FTerm> {
    context(
        "force",
        map(
            preceded(token("force"), pair(effect, cut(atom))),
                ...
                alt((
                    ...
                    map(
                        many0(pair(
                            ...
                            opt(preceded(token("="), cut(f_term))),
                        )),
                        Either::Left,
                    ),
                )),
            ),
            |(t, either)| match either {
                Either::Left(index_and_values) => index_and_values.into_iter().fold(
                    t,
                    |t, (index, assignment)| match assignment {
                        None => FTerm::MemGet {
                            base: Box::new(t),
                            offset: Box::new(index),
                        },
                        Some(value) => FTerm::MemSet {
                            base: Box::new(t),
                            offset: Box::new(index),
                            value: Box::new(value),
                        },
...
fn operator_id(
    operators: &'static [OperatorAndName],
) -> impl FnMut(Input) -> IResult<Input, FTerm> {
    map_token(|token| match token {
        Token::Normal(name, _, _) => operators
            .iter()
            .filter_map(|(op, fun_name)| if op == name { Some(fun_name) } else { None })
            .next()
            .map(|fun_name| FTerm::Identifier {
...
pub fn infixl<I, O1, O2, E, F, G>(
    mut operator: F,
    mut operand: G,
) -> impl FnMut(I) -> IResult<I, (O2, Vec<(O1, O2)>), E>
where
    I: Clone + InputLength,
    F: Parser<I, O1, E>,
    G: Parser<I, O2, E>,
...
pub fn infixr<I, O1, O2, E, F, G>(
    mut operator: F,
    mut operand: G,
) -> impl FnMut(I) -> IResult<I, (Vec<(O2, O1)>, O2), E>
where
    I: Clone + InputLength,
    F: Parser<I, O1, E>,
    G: Parser<I, O2, E>,
...
pub fn infix<I, O1, O2, E, F, G>(
    mut operator: F,
    mut operand: G,
) -> impl FnMut(I) -> IResult<I, (O2, Option<(O1, O2)>), E>
where
    I: Clone + InputLength,
    F: Parser<I, O1, E>,
    G: Parser<I, O2, E>,
...
fn operator_call(
    operators: &'static [OperatorAndName],
    fixity: Fixity,
    mut component: BoxedFTermParser,
) -> BoxedFTermParser {
    Box::new(move |input| match fixity {
        Infixl => context(
            "infixl_operator",
            map(
                infixl(
                    delimited(newline_opt, operator_id(operators), newline_opt),
                    |input| (*component)(input),
                ),
                |(head, rest)| {
                    rest.into_iter().fold(head, |acc, (op, arg)| FTerm::Redex {
                        function: Box::new(op),
                        args: vec![acc, arg],
                    })
                },
        ...
        Infixr => context(
            ...
            map(
                ...
                |(init, last)| {
                    init.into_iter().rfold(last, |acc, (arg, op)| FTerm::Redex {
                        function: Box::new(op),
                        args: vec![acc, arg],
                    })
                },
        ...
        Prefix => context(
            ...
            map(
                ...
                |(operator, operand)| match operator {
                    None => operand,
                    Some(operator) => FTerm::Redex {
                        function: Box::new(operator),
                        args: vec![operand],
                    },
                },
            ),
...
fn expr_impl(input: Input) -> IResult<Input, FTerm> {
    let mut p = PRECEDENCE
        .iter()
        .fold(scoped_app_boxed(), |f, (operators, fixity)| {
...
fn lambda(input: Input) -> IResult<Input, FTerm> {
    context(
        ...
        scoped(map(
            ...
            |(arg_names, effect, body)| FTerm::Lambda {
                arg_names,
                body: Box::new(body),
                effect,
            },
    ...
    context(
        ...
        scoped(map(
            tuple((
                ...
                c_type_decl,
                many0(map(
                    preceded(newline, scoped(tuple((int, preceded(token("=>"), f_term))))),
                    |(i, branch)| (i, branch),
                )),
                opt(preceded(
                    newline,
            ...
            |(t, result_type, branches, default_branch)| FTerm::CaseInt {
                t,
                result_type,
                branches,
                default_branch,
    ...
    context(
        ...
        map(
            tuple((
                alt((
                    scoped(pair(
                        delimited(token("let"), id, token("=")),
                        preceded(newline_opt, cut(f_term)),
                    )),
                    map(expr, |t| (String::from("_"), t)),
                )),
                preceded(newline, f_term),
            )),
            |((name, t), body)| FTerm::Let {
                name,
                t: Box::new(t),
                body: Box::new(body),
            },
            ...
            pair(
                many1(map(
                    scoped(tuple((
                        ...
                        c_type_decl,
                        preceded(
                            preceded(token("=>"), newline_opt),
                            map(cut(f_term), Box::new),
                        ),
                    ))),
...
fn handler_component(input: Input) -> IResult<Input, HandlerComponent> {
    alt((
        map(
            preceded(token("disposer"), cut(computation)),
            HandlerComponent::Disposer,
        ),
        map(
            preceded(token("replicator"), cut(computation)),
            ...
            |(((effect, parameter), handler_components), input)| {
                let mut handler = FTerm::Handler {
                    ...
                    transform: Box::new(FTerm::Lambda {
                        arg_names: vec![
                            ("p".to_owned(), VType::Uniform),
                            ("r".to_owned(), VType::Uniform),
                        ],
                        body: Box::new(FTerm::Identifier {
                            name: "r".to_owned(),
                            effect: Effect::Simple,
                        }),
                        effect: Effect::Simple,
                    ...
                    handlers: vec![],
                    input: Box::new(FTerm::Thunk {
                        computation: Box::new(input),
                        effect,
                    }),
                };
...
fn thunk(input: Input) -> IResult<Input, FTerm> {
    context(
        ...
        scoped(map(
            preceded(
                preceded(token("thunk"), newline_opt),
                pair(effect, cut(computation)),
            ),
            |(effect, t)| FTerm::Thunk {
                computation: Box::new(t),
                effect,
            },
        )),
    )(input)
...
pub fn parse_f_term(input: &str) -> Result<FTerm, String> {
    ...
    if input
        .tokens
        .iter()
        .any(|token| !matches!(token, Token::Indent(_, _)))
    {
        return Err(format!(
            "parse error: unexpected token at {:?}",
            input.tokens.first()
        ));
    }
    Ok(term)
